...
class GoogleScraper {
  ...
  async _scrapePage(searchQuery, limit = 100) {
    ...
    while (results.length < limit) {
      ...
      await page
        ...
        .catch(() => console.log("No results on this page")); // Unblock the flow
      ...
      if (previousCount === results.length) {
        console.log("End of the page is reached");
        break;
      }

      console.log(`Got ${results.length} results so far`);
    }
  ...
  async _scrollToEnd(page) {
    ...
    if (buttonIsVisible) {
      ...
      console.log("Clicked on show more results");
    }
  ...
  _parseLinksFromHTML(html) {
    ...
    $("#islrg div[jsaction][data-tbnid]").each(function (_i, containerElement) {
      ...
      const linkElementHref = containerElement_
        .find("a[href^='/imgres']")
        .attr("href");
